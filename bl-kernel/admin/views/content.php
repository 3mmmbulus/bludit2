		...
		foreach ($list as $pageKey) {
			try {
				...
				if (!$page->isChild()) {
					...
					echo '<a class="text-secondary d-none d-md-inline ms-2" href="'.HTML_PATH_ADMIN_ROOT.'edit-content/'.$page->key().'"><i class="bi bi-pencil-square"></i>'.$L->g('Edit').'</a>'.PHP_EOL;
					if (count($page->children())==0) {
						echo '<a href="#" class="ms-2 text-danger deletePageButton d-block d-sm-inline" data-bs-toggle="modal" data-bs-target="#jsdeletePageModal" data-key="'.$page->key().'"><i class="bi bi-trash"></i>'.$L->g('Delete').'</a>'.PHP_EOL;
					}
					echo '</td>';
					...
					foreach ($page->children() as $child) {
						...
						echo '<td class="contentTools pt-3 text-center d-sm-table-cell">'.PHP_EOL;
						if ($type=='published' || $type=='static' || $type=='sticky') {
						echo '<a class="text-secondary d-none d-md-inline" target="_blank" href="'.$child->permalink().'"><i class="bi bi-display"></i>'.$L->g('View').'</a>'.PHP_EOL;
						}
						echo '<a class="text-secondary d-none d-md-inline ms-2" href="'.HTML_PATH_ADMIN_ROOT.'edit-content/'.$child->key().'"><i class="bi bi-pencil-square"></i>'.$L->g('Edit').'</a>'.PHP_EOL;
						echo '<a class="ms-2 text-danger deletePageButton d-block d-sm-inline" href="#" data-bs-toggle="modal" data-bs-target="#jsdeletePageModal" data-key="'.$child->key().'"><i class="bi bi-trash"></i>'.$L->g('Delete').'</a>'.PHP_EOL;
						echo '</td>';
	...
	<div class="tab-pane show active" id="pages" role="tabpanel">
		...
		<nav class="paginator">
			...
			<li class="page-item <?php if (!Paginator::showPrev()) echo 'disabled' ?>">
				<a class="page-link" href="<?php echo Paginator::firstPageUrl() ?>"><span class="align-middle bi bi-skip-start-fill"></span> <?php echo $L->get('First'); ?></a>
			</li>
			...
			<!-- Last button -->
			<li class="page-item <?php if (!Paginator::showNext()) echo 'disabled' ?>">
				<a class="page-link" href="<?php echo Paginator::lastPageUrl() ?>"><?php echo $L->get('Last'); ?> <span class="align-middle bi bi-skip-end-fill"></span></a>
			</li>
