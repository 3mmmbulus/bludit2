...
EOF;
	...
	public static function link($args)
	{
		...
		if (isset($args['icon'])) {
			return '<a ' . $options . '><i class="bi bi-' . $args['icon'] . '"></i> ' . $args['title'] . '</a>';
		}
	...
	public static function pageTitle($args)
	{
		...
		return <<<EOF
<h2 class="mt-0 mb-3">
	<i class="bi bi-$icon" style="font-size: 0.9em;"></i> <span>$title</span>
</h2>
EOF;
	}
...
EOF;
	...
	public static function formInputTextBlock($args)
	{
		...
		$labelClass = 'form-label mt-4 mb-2 pb-2 border-bottom text-uppercase w-100';
...
EOF;
	...
	public static function formInputFile($args)
	{
		...
		$html .= '<input type="file" class="form-control" id="' . $id . '">';
		$html .= '</div>';
	...
	public static function formTextareaBlock($args)
	{
		...
		$html = '<div class="mb-3">';
		if (!empty($args['label'])) {
			$html .= '<label class="form-label mt-4 mb-2 pb-2 border-bottom text-uppercase w-100" for="' . $id . '">' . $args['label'] . '</label>';
		}
